...
for i in range(0, n+1):
	for j in range(0, n+1):
		v_y1 = (1.0*i/n + 1.0*(j%2)/(2*n)) * 0.95
		v_x1 = (1.0*j/n + 1.0*(i%2)/(2*n)) * 0.95
		v_y2 = 1.0*i/n
		v_x2 = 1.0*j/n
		alpha = 1.0*min(i, n-i, j, n-j)/(n/2.0);
		v_y[i][j] = alpha*v_y1 + (1.0-alpha)*v_y2
		v_x[i][j] = alpha*v_x1 + (1.0-alpha)*v_x2
...
def deform():
	...
	dx = (random.random() - 0.5) / (100*n)
	dy = (random.random() - 0.5) / (100*n)
...
#			pygame.draw.polygon(screen, (255,255,255), ((x1,y1),(x2,y2),(x3,y3),(x4,y4)), 1)
			...
			l_dens = local_densities(i+1,j+1)
			alpha = max(0.0, min(1.0, (l_dens[0]-min_dens)/(max_dens-min_dens)))
#			print (255*alpha,255*(1.0-alpha),0)
			pygame.draw.polygon(screen, (255*alpha,255*(1.0-alpha),0), ((x1,y1),(x2,y2),(x3,y3),(x4,y4)), 0)
#			pygame.draw.polygon(screen, (255,255,255), ((x1,y1),(x2,y2),(x3,y3),(x4,y4)), 1)
	pygame.display.flip()
	return
